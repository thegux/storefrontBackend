...
export default class OrderTable {
    async show(orderId: number):Promise<Order> {
        try {
            ...
            const sql_products = "SELECT * FROM products INNER JOIN order_products ON product.id = order_products.product_id WHERE order_products.order_id=($1)";
    ...
    async addProduct(quantity: number, orderId: number, productId: number):Promise<Order> {
        try {
            ...
            const databaseConnection = await Client.connect();
            const result = await databaseConnection.query(sql, [quantity, orderId, productId]);
            databaseConnection.release();
            return result.rows[0]
        } catch (error) {
