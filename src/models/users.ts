...
export default class UserTable {
    ...
    async create(newUser: User):Promise<User> {
        try {
            ...
            const databaseConnection = await Client.connect();
            const sql = "INSERT INTO users (first_name, last_name, password, username) VALUES($1,$2,$3) RETURNING *";
            const result = await databaseConnection.query(sql, [newUser.first_name, newUser.last_name, hash, newUser.username]);
            databaseConnection.release();
